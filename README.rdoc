...
- Install the plugin as described at: http://www.redmine.org/wiki/redmine/Plugins, according to the way you've obtained the plugin (don't forget to run the migration).
- Restart Redmine
...
- Create config/harvest.yml in your Redmine root like so:
    ...
    domain: [yourdomain].harvestapp.com
    email: [account email with api privileges]
    password: [guesswhat]
